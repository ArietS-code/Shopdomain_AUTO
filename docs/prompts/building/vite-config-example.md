...
Authoritative Vite configuration example. Copy exactly; adjust `appId` if needed.
...
Agent Instructions:
- Generate `vite.config.ts` identical to this example (remove markdown, keep code).
- Ensure `vue` alias matches bundler compiler build to prevent runtime compilation warnings and blank screens for inline template placeholders.